              ...
              <section className="mt-8 md:mt-12">
                ...
                <Link href={`/news/${featuredArticle.categorySlug}/${featuredArticle.slug}`} className="group block">
                  <GlassCard className="flex flex-col lg:flex-row gap-8 lg:gap-12 p-0 bg-white border border-transparent shadow-[0_4px_24px_rgba(0,0,0,0.04)] rounded-3xl overflow-hidden transition-all duration-500 group-hover:-translate-y-1.5 group-hover:shadow-[0_12px_32px_rgba(0,0,0,0.08)]">
                    {/* Cover Image Wrapper */}
                    <div className="relative w-full lg:w-3/5 aspect-[4/3] lg:aspect-[16/9] overflow-hidden bg-slate-50/80 border-r border-slate-100">
                      <Image
                        src={encodeURI(featuredArticle.coverImage || (featuredArticle as { image?: string }).image || "/assets/images/logo%20kimxwed.png")}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
                {regularArticles.map((article) => (
                  ...
                  >
                    ...
                    >
                      {/* Image Container */}
                      <div className={`relative w-full aspect-[4/3] overflow-hidden transition-all duration-700 ease-in-out ${article.coverFit === 'contain' ? 'bg-slate-950 border-b border-sky-100/50' : 'bg-slate-50'}`}>
